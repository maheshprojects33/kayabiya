...
class BaseLoanListView(ListView):
    model = Loan
    context_object_name = 'loans'
...
class LoanRequestView(BaseLoanListView):
    ...
    def get_queryset(self):
        ...
        return Loan.objects.filter(status__in=['Pending', 'Approved'])
...
class LoanDisbursedView(BaseLoanListView):
    ...
    def get_queryset(self):
        # Filter loans based on the status
        return Loan.objects.filter(status='Disbursed').exclude(has_penalty="True")
...
class LoanSettledView(BaseLoanListView):
    ...
    def get_queryset(self):
        # Filter loans based on the status
        return Loan.objects.filter(status='Settled')
...
class LoanVoidedView(BaseLoanListView):
    ...
    def get_queryset(self):
        # Filter loans based on the status
        return Loan.objects.filter(status__in=['Cancelled', 'Rejected'])
